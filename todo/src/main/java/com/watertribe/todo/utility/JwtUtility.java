...
public class JwtUtility {
  ...
  public boolean validateToken(String token, UserDetails userDetails) {
    ...
    // 2. Validate that the username in the token matches the user details
    // AND that the token itself is valid/not expired
  ...
  public boolean validateToken(String token) {
    try {
      ...
      /*
       * We catch JwtException to handle any problems with the JWT itself, such as
       * being expired,
       * malformed, or tampered with.
       * 
       * We catch IllegalArgumentException to handle issues with the token not being
       * present or empty,
       * any any other issue that would cause the parser to be unable to interact with
       * it
       */
      return false;
    }
